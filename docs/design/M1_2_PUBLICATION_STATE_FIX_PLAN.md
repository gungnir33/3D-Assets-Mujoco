...
- [x] 真实小箱体+engineering_static_v1跑native，仅注入render_package异常和发布spy。先断言磁盘/checked_report均physics passed、scope verified，unavailable时CLI7、failed时FAILED；assert not published且staging存在。
- [x] 运行定向pytest，记录旧摘要冲突失败。
- [x] native完成后采用持久化完整result；异常仅更新render，绑定失败证据，保留原错误与目录。诊断I/O抛EvidenceIOError并保留因果，不掩盖初始render异常。
- [x] 测试异常矩阵和acceptance不冒充通过，提交本地修复。

## 任务2：统一发布门槛

文件：pipeline.py；测试tests/integration/test_publication_gate.py。

- [x] 渲染后故障注入修改真实contact结果、去掉必需证据层/资源；不得mock门槛或原native。记录convert错误发布的失败回归。
- [x] `result=checked_report(staging)`后按请求compile/physics/full/visual检查必需层、scope、证据问题；失败抛结构化ValidationFailed（分类/阶段/原因/result/package），成功才atomic_publish，保持原并发防覆盖实现。
- [x] 每项拒绝都断言无成功目录、未调用publish、staging保留和准确原因；正常compile/visual/physics/full均真实运行。
- [x] 定向及完整源码回归，本地提交。

## 任务3：安装、真实复测、审核和报告

- [x] `python -m pip install --no-deps --no-build-isolation .`仅重装本项目；逐文件校验site-packages后运行完整pytest与pip check。
- [x] 原GLB两种profile在新唯一目录各跑一次acceptance，保留原请求，候选迁移并核对分层报告。保存新数值、状态、命令、退出码和保护快照。
- [x] 一次独立只读整体审核；必要问题先失败测试再最小修复，不重复参数诊断。
- [x] 更新README、权威设计局部契约、M1_2_PUBLICATION_STATE_FIX_REPORT.md和部署记录，保留历史；分阶段本地提交后停止。
