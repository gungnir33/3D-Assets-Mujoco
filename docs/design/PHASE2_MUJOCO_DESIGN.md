...
当前唯一权威设计为本文件。原稿按字节归档为 `PHASE2_MUJOCO_DESIGN.original.md`，仅用于追溯，不继续维护。第二阶段初始化后，将本文件迁入 `/home/mcl/workspace/3D-Assets-Mujoco/docs/design/PHASE2_MUJOCO_DESIGN.md`，该路径成为唯一权威版本；原 tasks 文件改为迁移指针（路径、版本、提交和 SHA256），不保留两份可编辑正文。修订说明、原稿与修改记录同期归档。本轮已授权独立第二阶段环境的任务 1–7，不修改第一阶段或现有宿主环境，不调用生成、不自动 push。
