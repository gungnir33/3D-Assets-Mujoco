...
def test_real_m1(tmp_path):
    ...
    with pytest.raises(ValidationFailed) as exc:
        convert(ConversionRequest(input=source,output=tmp_path/"out",name="penguin",source_up="y",yaw_deg=180,scale=.5,validation_level="full"))
    ...
    assert evidence["native"]["max_penetration_m"]>evidence["native"]["penetration_limit_m"]
