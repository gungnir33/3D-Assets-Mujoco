...
for profile in ('preserve','engineering_static_v1'):
    ...
    summary['profiles'][profile]={'command':cmd,'exit_code':run.returncode,'report_exit_code':cli.returncode,'package':str(package),'report_consistency':equal,'status':report['status'],'native':report['native'],'followup_ground':report['followup_ground'],'migration':report['migration'],'package_sha256':fingerprint(package)}
    assert equal and run.returncode==(5 if profile=='preserve' else 0)
    assert cli.returncode==run.returncode
