...
for kind in ('render_unavailable','render_failed','diagnostic_io','scope_contradiction','render_status_corrupt'):
    ...
    def status_failure(path,*args,**kwargs):
        value=write(path,*args,**kwargs)
        if path.name=='conversion.log':
            evidence_path=path.parent/'evidence_manifest.json'
            evidence=json.loads(evidence_path.read_text())
            evidence['layers']['render']['status']='not_run'
    ...
    with contextlib.ExitStack() as stack:
        ...
        elif kind!='render_status_corrupt':
        ...
        if kind=='render_status_corrupt':
            stack.enter_context(patch.object(pathlib.Path,'write_text',status_failure))
    ...
    expected={'render_unavailable':7,'render_failed':5,'diagnostic_io':3,'scope_contradiction':5,'render_status_corrupt':5}[kind]
    assert code==expected and report['physics']=='passed'
    if kind in ('render_unavailable','render_failed'):
